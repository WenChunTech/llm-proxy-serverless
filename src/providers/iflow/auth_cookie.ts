...
async function fetchAPIKeyInfo(cookie: string): Promise<iFlowKeyData> {
    ...
    if (!response.ok) {
        ...
        throw new Error(`[iFlow Cookie Auth] GET request failed: ${response.status} ${errorText}`);
    }

    const result: iFlowAPIKeyResponse = await response.json();

    if (!result.success) {
        throw new Error(`[iFlow Cookie Auth] GET request not successful: ${result.message}`);
    }
 ...
 */
async function refreshAPIKey(cookie: string, name: string): Promise<iFlowKeyData> {
    const headers = {
        "Cookie": cookie,
    ...
    if (!response.ok) {
        const errorText = await response.text();
        throw new Error(`[iFlow Cookie Auth] POST request failed: ${response.status} ${errorText}`);
    }

    const result: iFlowAPIKeyResponse = await response.json();

    if (!result.success) {
        throw new Error(`[iFlow Cookie Auth] POST request not successful: ${result.message}`);
    }
