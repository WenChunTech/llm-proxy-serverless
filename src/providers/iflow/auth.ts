...
async function updateAndStoreCredentials(
  originalAuth: IFlowAuth,
  updatedCreds: IFlowAuth,
): Promise<IFlowAuth> {
  const newConfig = {
    ...appConfig,
    iflow: appConfig.iflow.map((c) =>
      c.auth?.refresh_token === originalAuth.refresh_token
        ? { ...c, auth: updatedCreds }
        : c
    ),
  };
...
export async function refreshAccessToken(auth: IFlowAuth) {
  if (auth.cookie) {
    try {
      ...
      console.error(
        "[iFlow Auth] Failed to refresh access token with cookie. error:",
        error,
      );
    }
  }
  ...
  if (!newTokenData.access_token || !newTokenData.refresh_token) {
    console.error(
      "[iFlow Auth] Missing access token or refresh token in refresh response. error:",
      newTokenData,
    );
    return auth;
  }
...
async function fetchUserInfo(accessToken: string) {
  ...
  if (!response.ok) {
    const errorText = await response.text();
    throw new Error(
      `[iFlow Auth] User info request failed: ${response.status} ${errorText}`,
    );
  }
  ...
  if (!result.success || !result.data || !result.data.apiKey) {
    ...
    throw new Error(
      "[iFlow Auth] User info request not successful or API key missing.",
    );
  }
  return result.data;
}
