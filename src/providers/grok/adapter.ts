...
export function convertToGrokRequestTo(body: any, source: any) {
  switch (source) {
    ...
    default:
      throw new Error(
        `Unsupported source type for Grok providerType: ${source}`,
      );
  }
}

export async function convertGrokResponseTo(
  c: any,
  response: Response,
  target: any,
) {
...
export async function convertGrokStreamResponseTo(
  stream: any,
  response: Response,
  target: any,
