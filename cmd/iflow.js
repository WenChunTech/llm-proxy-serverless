...
async function fetchToken(body) {
  ...
  if (!response.ok) {
    ...
    throw new Error(
      `iFlow token request failed: ${response.status} ${errorText}`,
    );
  }
...
async function fetchUserInfo(accessToken) {
  ...
  if (!response.ok) {
    const errorText = await response.text();
    throw new Error(
      `iFlow user info request failed: ${response.status} ${errorText}`,
    );
  }
  ...
  if (!result.success || !result.data || !result.data.apiKey) {
    throw new Error(
      "iFlow user info request not successful or API key missing in response.",
    );
  }
  return result.data;
}
...
async function getNewTokenFromWeb() {
  ...
  return new Promise((resolve, reject) => {
    ...
    const authUrl =
      `${IFLOW_OAUTH_AUTHORIZE_ENDPOINT}?${authUrlParams.toString()}`;
    ...
    const server = http.createServer(async (req, res) => {
      try {
        ...
        if (code) {
          ...
          console.log(
            `[iFlow Auth] New token stored successfully in ${finalCredPath}`,
          );
          resolve(credentials);
        } else {
        ...
        console.error(
          "[iFlow Auth] An error occurred during the authentication process:",
          e,
        );
        server.close();
        reject(e);
