...
async function discover() {
    ...
    if (!resp.ok) {
        throw new Error(`xAI discovery failed: ${resp.status} ${await resp.text()}`);
    }
    const data = (await resp.json());
    if (!data.authorization_endpoint || !data.token_endpoint) {
        throw new Error("xAI discovery response missing authorization_endpoint or token_endpoint");
    }
    return {
    ...
    return new Promise((resolve, reject) => {
        ...
        const server = createServer((req, res) => {
            ...
            if (code && !error) {
                ...
                res.end(`<h1>Login failed</h1><p>Error: ${error || "unknown"}</p><p>Please check the CLI output.</p>`);
            }
        });
...
Options:
  ...
  --base-url=<URL>      Override the default API base URL (default: https://api.x.ai/v1)
...
`);
    ...
    const outputDir = args.find((a) => a.startsWith("--output-dir="))?.split("=")[1] || DEFAULT_OUTPUT_DIR;
    const models = args.find((a) => a.startsWith("--models="))?.split("=")[1]?.split(",").filter(Boolean) || [
        "grok-3-mini",
        "grok-3-mini-fast",
    ];
    const baseUrl = args.find((a) => a.startsWith("--base-url="))?.split("=")[1] || DEFAULT_BASE_URL;
    const headersArg = args.find((a) => a.startsWith("--headers="))?.split("=").slice(1).join("=");
    let customHeaders;
    if (headersArg) {
        try {
            ...
            console.error("Invalid --headers JSON. Example: --headers=\'{\"x-grok-client-version\":\"0.2.93\"}\'");
            process.exit(1);
        }
    }
    ...
    console.log(`  Callback server listening on http://${REDIRECT_HOST}:${actualPort}`);
    ...
    console.log("  If the browser shows a code instead of redirecting, paste it below.\n");
    ...
    const manualPromptDelay = new Promise((resolve) => setTimeout(resolve, MANUAL_PROMPT_DELAY_MS));
    let authCode = null;
    try {
        await Promise.race([callbackPromise, timeoutPromise, (async () => {
            await manualPromptDelay;
            while (!callbackResolved && !manualAborted) {
                const input = await readLineFromStdin("Paste the code from the browser (or press Enter to keep waiting): ");
                if (callbackResolved || manualAborted)
                    break;
                if (!input)
                    continue;
    ...
    if (authOutput.sub)
        console.log(`  Subject: ${authOutput.sub}`);
    ...
    const fileName = safeName !== "unknown" ? `grok-${safeName}.json` : `grok-${Date.now()}.json`;
