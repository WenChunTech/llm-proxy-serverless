...
2. 添加您的 LLM 提供商的 API 密钥和任何其他配置。该文件应遵循以下结构：
   ...
   ```json
   {
     ...
     "model_priority": ["gemini_cli", "iflow", "openai_chat", "openai_responses", "qwen", "claude"],
     "fallback_models": {
...
```bash
deno serve --allow-all --unstable-kv --unstable-cron --watch src/index.ts
```

或生产环境：

```bash
deno serve --allow-net --allow-import --allow-read --allow-env --unstable-kv --unstable-cron src/index.ts
```
...
2. **提供商选择** — `src/providers/registry.ts` 维护统一 provider 元数据，`src/providers/factory.ts`
   基于 registry 根据模型名称查找可用提供商，并按 `model_priority` 配置选择优先级最高的提供商。
...
- **配置加载**: 启动时从 `config.json` 文件或 Deno KV
  加载配置。配置更新时会通过统一运行时刷新路径重建 poller，并自动清除内部路由缓存。
